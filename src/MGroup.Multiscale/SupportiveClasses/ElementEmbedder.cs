...
namespace MGroup.MSolve.Discretization.Embedding
{
	public class ElementEmbedder : IElementDofEnumerator
	{
		...
		private void InitializeMappings()
		{
			...
			{
				var list = (IReadOnlyList<INode>)embeddedElement.Nodes;

				int nodeOrderInEmbeddedElement = list.FindFirstIndex(node); //TODO: Fix that. It was embeddedElement.Nodes.IndexOf(embeddedNode.Node)
		...
		public void CalculateTransformationMatrix()
		{
			...
			foreach (var node in embeddedElement.Nodes.Except(e.EmbeddedNodes.Select(x => x.Node)))
			{
				...
				int nodeOrderInEmbeddedElement = list.FindFirstIndex(node);
